...
{
    [SerializeField]
    private GameObject _SmeasurementPointPrefab;
    [SerializeField]
    private GameObject _EmeasurementPointPrefab;
    ...
    void Awake()
    {
        ...
        startPoint = Instantiate(_SmeasurementPointPrefab, Vector3.zero, Quaternion.identity);
        endPoint = Instantiate(_EmeasurementPointPrefab, Vector3.zero, Quaternion.identity);
    ...
    {
        if (_SmeasurementPointPrefab == null || _EmeasurementPointPrefab == null)
        {
    ...
    void Update()
    {
        ...
        if (startPoint.activeSelf && endPoint.activeSelf)
        {
           // distanceText.transform.position = endPoint.transform.position + offsetMeasurement;
            ...
            distanceText.text = $"Distance: {(Vector3.Distance(startPoint.transform.position, endPoint.transform.position) * measurementFactor).ToString("F3")} M";
        }
    }
